...
> **مهم**: این پیاده‌سازی با اپلیکیشن **Expo Go** کار نمی‌کند. برای عملکرد صحیح پلاگین پرداخت مایکت، باید اپلیکیشن را مستقیماً روی دستگاه یا شبیه‌ساز بیلد و نصب کنید.

## پیش‌نیازها

1. **حساب توسعه‌دهنده مایکت**: نام پکیج اپلیکیشن خود را در پنل توسعه‌دهندگان مایکت ثبت کرده و کلید عمومی RSA را دریافت کنید.
...
> **توجه**: در کد بالا، عبارت `YOUR_RSA_PUBLIC_KEY` را با کلید عمومی RSA که از پنل توسعه‌دهندگان مایکت دریافت کرده‌اید جایگزین کنید.
...
- **فونت Vazir**: در کد نمونه، از فونت `Vazir` استفاده شده است. اطمینان حاصل کنید که این فونت در پروژه شما تعریف شده باشد.
...
برای مشاهده یک نمونه کامل از پیاده‌سازی پرداخت مایکت، می‌توانید نمونه پروژه را از [اینجا](https://github.com/myket-developer/sample) دانلود کنید.